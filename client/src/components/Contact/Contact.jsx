...
//         if (!iframeRef.current || isMapLoading) return;
//
...
//
//         // Устанавливаем src для загрузки карты
//         const cleanMapUrl = MAP_EMBED_URL.split('?')[0];
//         const newSrc = `${cleanMapUrl}?hl=ru&z=15`;
//         iframe.src = newSrc;
//     }, [MAP_EMBED_URL, isMapLoading]);
//
...
//         }
//
//         // Короткая задержка перед перезагрузкой
//         setTimeout(() => {
//             if (iframeRef.current) {
//                 const iframe = iframeRef.current;
//                 const cleanMapUrl = MAP_EMBED_URL.split('?')[0];
//                 // Добавляем timestamp для обхода кеша
//                 iframe.src = `${cleanMapUrl}?hl=ru&z=15&t=${Date.now()}`;
//
...
//                                 {!mapLoaded && !mapError && isMapLoading && (
//                                     <div className="contact-map-loading">
